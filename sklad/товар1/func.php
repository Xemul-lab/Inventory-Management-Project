...
if (isset($_POST['submit'])) {
	$sql = ("INSERT INTO `tovar1`(`name`, `namename`, `code`,  `quantityskl` , `quantitysold`, `price`)VALUES (?,?,?,?,?,?)");
...
$sql = $pdo->prepare("SELECT * FROM `tovar1`");
$sql->execute();
$result = $sql->fetchAll();
...
if (isset($_POST['edit-submit'])) {
	$sqll = "UPDATE tovar1 SET name=?, namename=?, code=?, quantityskl=?, quantitysold=?, price=? WHERE id=?";
...
if (isset($_POST['delete_submit'])) {
	$sql = "DELETE FROM tovar1 WHERE id=?";
	$query = $pdo->prepare($sql);
