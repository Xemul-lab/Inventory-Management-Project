...
if (isset($_POST['submit'])) {
	$sql = ("INSERT INTO `tovar2`(`name`, `namename`, `code`,  `quantityskl` , `quantitysold`, `price`)VALUES (?,?,?,?,?,?)");
	$query = $pdo->prepare($sql);
...
$sql = $pdo->prepare("SELECT * FROM `tovar2`");
$sql->execute();
$result = $sql->fetchAll();
...
if (isset($_POST['edit-submit'])) {
	$sqll = "UPDATE tovar2 SET name=?, namename=?, code=?, quantityskl=?, quantitysold=?, price=? WHERE id=?";
...
if (isset($_POST['delete_submit'])) {
	$sql = "DELETE FROM tovar2 WHERE id=?";
	$query = $pdo->prepare($sql);
